...
0. A standalone fix (patches 1-2, independently applicable): the exit
   ...
   reproducers in `repro/`). The replay fires only while
   `FUTEX_WAITERS` is not re-armed: an observed bit is only ever
   cleared by an unlock which wakes, so it already guarantees a
   wakeup from the live owner - a meaning of the bit the exit walk
   itself relies on (it preserves the bit when marking a dead owner's
   futex and keys its own wakeup on it).
...
- Kernel selftests: 39 futex tests (including a live PID namespace
